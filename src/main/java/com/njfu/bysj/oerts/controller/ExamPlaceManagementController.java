...
public class ExamPlaceManagementController {
	...
	@PostMapping("/examplace/delete")
	public JsonResult deleteExamPlace(@RequestBody ExamPlaceManagementEntity delete) {
		examPlaceManagementService.deleteExamPlace(delete);
		return JsonUtil.success();
	}

	@PostMapping("/examplace/add")
	public JsonResult addExamPlace(@RequestBody ExamPlaceManagementEntity add) {
		if (examPlaceManagementService.addExamPlace(add)) {
			return JsonUtil.success();
		} else {
			return JsonUtil.failed("已经有相同的考试地址了");
		}
	}
}
